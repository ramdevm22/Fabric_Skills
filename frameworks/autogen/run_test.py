"""
AutoGen (Microsoft) test harness for Fabric skill benchmark.
...
Why AutoGen: Built by Microsoft, directly relevant to MS Fabric ecosystem.
Supports agentic workflows natively.
...
def save_to_results(model, elapsed, response_text, grade):
    ...
    with open(RESULTS_PATH, "a", encoding="utf-8") as f:
        f.write(f"| {timestamp} | AutoGen (Microsoft) | {model} | {elapsed:.2f} | {grade} |\n")
        f.write(f"\n---\n### Run: {timestamp} | AutoGen | {model}\n")
...
async def run_agent(skill_text, user_prompt, api_key):
    """
    AutoGen pattern: model client + AssistantAgent + single message
    """
...
def main():
    ...
    print(f"\nModel     : {MODEL}")
    print(f"Framework : AutoGen (Microsoft)")
